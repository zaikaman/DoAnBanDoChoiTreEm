...
public class tables {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            st.executeUpdate("insert into nguoidung (userRole,name,mobileNumber,email,password,address,status) values('Nhan Vien','Sang','123456','nv1','password1','Vietnam','true')");
            st.executeUpdate("insert into nguoidung (userRole,name,mobileNumber,email,password,address,status) values('Nhan Vien','Quốc','123457','nv2','password2','Vietnam','true')");
            st.executeUpdate("insert into nguoidung (userRole,name,mobileNumber,email,password,address,status) values('Nhan Vien','Thịnh','123458','nv3','password3','Vietnam','true')");
            rs = st.executeQuery("SHOW TABLES LIKE 'khachhang'");
            if (!rs.next()) {
                st.executeUpdate("create table khachhang(khachhang_pk int AUTO_INCREMENT primary key, name varchar(200), emailNumber varchar(200), address varchar(200), mobileNumber varchar(50))");
            }
            for (int i = 1; i <= 10; i++) {
                st.executeUpdate("insert into khachhang (name, emailNumber, address, mobileNumber) values('Khach Hang " + i + "','kh" + i + "@mail.com','Dia Chi " + i + "','12345" + i + "')");
            }
            rs = st.executeQuery("SHOW TABLES LIKE 'sanpham'");
            if (!rs.next()) {
                st.executeUpdate("create table sanpham(sanpham_pk int AUTO_INCREMENT primary key, ten varchar(200), xuatxu varchar(200), gia decimal(10,2), soluong int)");
            }
